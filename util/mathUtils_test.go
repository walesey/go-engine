...
func TestSegmentCircleIntersect_intersectionEntry(t *testing.T) {
	intersect, err := SegmentCircleIntersect(0.5, Vector2{2, 1}, Vector2{1, 3}, Vector2{2, 1})
	assert.Nil(t, err, "SegmentCircleIntersect error should be nil")
	expectApproxEqual2D(t, Vector2{1.776, 1.447}, intersect)
}

func TestSegmentCircleIntersect_intersectionExit(t *testing.T) {
	intersect, err := SegmentCircleIntersect(0.5, Vector2{2, 1}, Vector2{2, 1}, Vector2{1, 3})
	assert.Nil(t, err, "SegmentCircleIntersect error should be nil")
	expectApproxEqual2D(t, Vector2{1.776, 1.447}, intersect)
}

func TestSegmentCircleIntersect_2intersections(t *testing.T) {
	intersect, err := SegmentCircleIntersect(0.5, Vector2{2, 1}, Vector2{1, 3}, Vector2{4, -2})
	assert.Nil(t, err, "SegmentCircleIntersect error should be nil")
	expectApproxEqual2D(t, Vector2{1.905, 1.491}, intersect)
}

func TestSegmentCircleIntersect_noIntersection(t *testing.T) {
	_, err := SegmentCircleIntersect(0.5, Vector2{2, 1}, Vector2{1, 1}, Vector2{5, -2})
